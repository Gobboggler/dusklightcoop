...
static void renderSceneForWindow(int windowIdx, J2DOrthoGraph& ortho) {
    dDlst_window_c* window_p = dComIfGp_getWindow(windowIdx);
...
int mDoGph_Painter() {
    ...
    if (dComIfGp_getWindowNum() != 0) {
        int playerCount = 1;
#if TARGET_PC
        if (dusk::getSettings().backend.enabled.getValue()) {
            playerCount = dusk::getSettings().backend.splitScreenPlayerCount.getValue();
        }
#endif
        for (int p = 0; p < playerCount; p++) {
#if TARGET_PC
            if (playerCount > 1) {
                auto [vx, vy, vw, vh] = dusk::getSplitscreenViewport(
                    p, playerCount,
                    static_cast<dusk::SplitscreenLayout>(
                        dusk::getSettings().backend.splitScreenLayout.getValue()));
                GXSetViewport(vx, vy, vw, vh, 0.0f, 1.0f);
                GXSetScissor(vx, vy, vw, vh);
            }
#endif
            int windowIdx = (p < dComIfGp_getWindowNum()) ? p : 0;
            renderSceneForWindow(windowIdx, ortho);
        }
    }
